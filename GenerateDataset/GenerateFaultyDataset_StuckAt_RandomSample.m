...
PhaseDelay = [0,1,2,3,4];
faultDuration = [8,10,12];
%faultValue = [50,70,90];

fname = strcat(scenario, "_StuckAt_Randomsample");
...
% randomIndex_FaultValue = randi(length(faultValue), 1);
% SpeedSensorFaultValueV2.set('Value', string(faultValue(randomIndex_FaultValue)));
SpeedSensorFaultEventV2.set('Value', 'Deterministic');
randomIndex_Phase_Delay = randi(length(PhaseDelay), 1);
SpeedSensorEventValueV2.set('Value', string(PhaseDelay(randomIndex_Phase_Delay)));
SpeedSensorFaultEffectV2.set('Value', 'Constant time');
randomIndex_Fault_Dur = randi(length(faultDuration), 1);
SpeedSensorEffectValueV2.set('Value', string(faultDuration(randomIndex_Fault_Dur)));
...
% randomIndex_FaultValue = randi(length(faultValue), 1);
% DistanceSensorFaultValueV2.set('Value', string(faultValue(randomIndex_FaultValue)));
DistanceSensorFaultEventV2.set('Value', 'Deterministic');
randomIndex_Phase_Delay = randi(length(PhaseDelay), 1);
DistanceSensorEventValueV2.set('Value', string(PhaseDelay(randomIndex_Phase_Delay)));
DistanceSensorFaultEffectV2.set('Value', 'Constant time');
randomIndex_Fault_Dur = randi(length(faultDuration), 1);
DistanceSensorEffectValueV2.set('Value', string(faultDuration(randomIndex_Fault_Dur)));
...
% randomIndex_FaultValue = randi(length(faultValue), 1);
% SpeedSensorFaultValueV6.set('Value', string(faultValue(randomIndex_FaultValue)));
SpeedSensorFaultEventV6.set('Value', 'Deterministic');
randomIndex_Phase_Delay = randi(length(PhaseDelay), 1);
SpeedSensorEventValueV6.set('Value', string(PhaseDelay(randomIndex_Phase_Delay)));
SpeedSensorFaultEffectV6.set('Value', 'Constant time');
randomIndex_Fault_Dur = randi(length(faultDuration), 1);
SpeedSensorEffectValueV6.set('Value', string(faultDuration(randomIndex_Fault_Dur)));

%setting up the FI block for injecting noise fault at V6
%distance sensor
DistanceSensorFInjEnV6.set('Value', 'on');
DistanceSensorFaultTypeV6.set('Value', 'Stuck-at');
% randomIndex_FaultValue = randi(length(faultValue), 1);
% DistanceSensorFaultValueV6.set('Value', string(faultValue(randomIndex_FaultValue)));
DistanceSensorFaultEventV6.set('Value', 'Deterministic');
randomIndex_Phase_Delay = randi(length(PhaseDelay), 1);
DistanceSensorEventValueV6.set('Value', string(PhaseDelay(randomIndex_Phase_Delay)));
DistanceSensorFaultEffectV6.set('Value', 'Constant time');
randomIndex_Fault_Dur = randi(length(faultDuration), 1);
DistanceSensorEffectValueV6.set('Value', string(faultDuration(randomIndex_Fault_Dur)));
